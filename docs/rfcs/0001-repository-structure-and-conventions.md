...
| Integration | `crates/moraine/tests/` | Public API against real SlateDB on in-memory `object_store`. **No mocks of the store layer.** Covers snapshot visibility, concurrent-commit conflicts, and the crash cases RFC 0011 names — each pinned to what must hold after reopen, and driven as data so a case cannot be silently dropped. | every push/PR |
