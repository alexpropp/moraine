...
use crate::{
    error::Error,
    frame,
    proto::{CommitValue, EnvelopeValue, SlotPayloadValue, SlotWriteValue},
};

/// The width of a committer-minted transaction id.
const TRANSACTION_ID_LEN: usize = 16;

/// One slot's content: the commits that won it, in order.
...
mod tests {
    ...
    fn any_envelope() -> impl Strategy<Value = Envelope> {
        ...
        proptest::collection::vec(commit, 0..6).prop_map(|commits| Envelope { commits })
    }
