...
// any error. Throws on a NULL, and on a type equality indexes do not cover.
MoraineLookupValue BuildLookupValue(const duckdb::Value &value, LookupValueBacking &backing, const char *caller) {
	// A typed NULL would otherwise reach a `GetValue` that raises an internal
	// error; an untyped one lands in `default` naming the SQLNULL type.
	if (value.IsNull()) {
		throw duckdb::InvalidInputException(
		    "%s: NULL is not a value to match; use moraine_index_nulls for an IS NULL query", caller);
	}
	...
	auto build_bound = [](const duckdb::Value &value, std::vector<LookupValueBacking> &backing,
		...
		if (value.type().id() == duckdb::LogicalTypeId::STRUCT) {
			...
			for (idx_t i = 0; i < children.size(); i++) {
				out.push_back(BuildLookupValue(children[i], backing[i], "moraine_index_range"));
			}
		} else {
			backing.resize(1);
			out.push_back(BuildLookupValue(value, backing[0], "moraine_index_range"));
		}
	};
