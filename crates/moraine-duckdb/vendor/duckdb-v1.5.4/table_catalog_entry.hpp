...
// https://raw.githubusercontent.com/duckdb/duckdb/v1.5.4/src/include/duckdb/catalog/catalog_entry/table_catalog_entry.hpp
// (git tag v1.5.4, commit 08e34c447b). Same amalgamation gap as
// `storage_extension.hpp`: the amalgamation never reaches
// `duckdb/catalog/catalog_entry/table_catalog_entry.hpp`. Its includes
// (`standard_entry.hpp`, `unordered_map.hpp`, `column_list.hpp`,
// `constraint.hpp`, `bound_constraint.hpp`, `table_statistics.hpp`,
// `expression.hpp`, `case_insensitive_map.hpp`, `table_column_type.hpp`,
// `column_dependency_manager.hpp`, `table_column.hpp`) are all already
// fully defined in `duckdb.hpp` (the class body below never names
// `ColumnDependencyManager` directly, so its header need not be vendored).
// Kept byte-for-byte identical to upstream — see `create_schema_info.hpp`
// for why. Must be included after `duckdb.hpp` and `create_table_info.hpp`.
#pragma once
